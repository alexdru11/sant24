...
static Stanza *ultimaStanza = NULL; // Puntatore all'ultima stanza della mappa
...
static int combatti(Giocatore *giocatore, int tipo_nemico);
...
// Funzione per inserire una stanza in coda alla mappa
static void ins_stanza() {
    ...
    if (primaStanza == NULL) {
        ...
        ultimaStanza = nuova_stanza;
    } else {
        ultimaStanza->stanza_successiva = nuova_stanza;
        ultimaStanza = nuova_stanza;
    }
}
...
static void canc_stanza() {
    ...
    Stanza *penultimaStanza = primaStanza;
    while (penultimaStanza->stanza_successiva != ultimaStanza) {
        penultimaStanza = penultimaStanza->stanza_successiva;
    }

    free(ultimaStanza);
    ultimaStanza = penultimaStanza;
    ultimaStanza->stanza_successiva = NULL;
}
...
static void stampa_stanze() {
    ...
    while (current != NULL) {
        ...
        current = current->stanza_successiva;
        ...
        do {
            ...
            switch (scelta) {
                case 1:
                    ...
                    break;
                case 2:
                   if(combatti(giocatore_corrente, -1)){
                        gioco_finito = 1;
                   }
                   azione_eseguita = 1;
                    break;
...
static void avanza(Giocatore *giocatore) {
    ...
    if(stanza_corrente->stanza_successiva != NULL){
        //Sposta il giocatore nella stanza successiva
        sposta_giocatore(giocatore, stanza_corrente->stanza_successiva);

        // Applica l'effetto del tesoro
        if(stanza_corrente->stanza_successiva->tesoro != NESSUN_TESORO){
        ...
        if (rand() % 100 < 25) {
            ...
            int tipo_nemico = (rand() % 100 < 60) ? 0 : 1; // 0: scheletro, 1: guardia
            if(combatti(giocatore, tipo_nemico)){
...
static void scappa(Giocatore *giocatore) {
    ...
    if(giocatore->classe == PRINCIPE){
        if(giocatore->fuggito == 0){
            ...
            while(current->stanza_successiva != giocatore->posizione){
                current = current->stanza_successiva;
            }

            sposta_giocatore(giocatore, current);
            giocatore->fuggito++;
        } else {
...
static void cerca_porta_segreta(Giocatore *giocatore) {
    ...
    if(stanza_corrente->stanza_successiva == NULL) direzione = 3;
    ...
    if ((rand() / (float)RAND_MAX) < probabilita) {
        ...
        // Collega temporaneamente la nuova stanza alla stanza corrente
        stanza_corrente->stanza_successiva = nuova_stanza;
        ...
        //Scollega la stanza segreta
        stanza_corrente->stanza_successiva = NULL;
        free(nuova_stanza);
...
static void libera_mappa() {
    Stanza *current = primaStanza;
    while (current != NULL) {
        Stanza *next = current->stanza_successiva;
        free(current);
        current = next;
    }
    primaStanza = NULL;
